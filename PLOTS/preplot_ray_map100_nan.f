...
c      zrad=z0+(ilayer-1)*dz-dz/2.0d0
      zrad=z0+(ilayer-1)*dz
      if (ivelin.ne.2.and.ivelin.ne.3.and.ivelin.ne.5) then
      ...
      do i=1,ny
       do j=1,nx
        ...
        if (image(iface).lt.999.0d0) then                                                                                              
            ivtot=ivtot+1
            image(iface)=-image(iface)*c*100.
c        write(6,*)image(iface)
        vmean=vmean+image(iface)
        endif
       enddo
      enddo
c      vmean=vmean/dble(nface)
      vmean=vmean/dble(ivtot)
      ...
      do i=1,ny
       ...
       do j=1,nx
        ...
        if (image(iface).lt.999.0d0) then
        write(1,'(2f10.4,3e12.4)')lon1,lat1,
     &  image(iface)-vmean,image(iface),(1.+image(iface)*0.01)*c
        else
         write(1,'(2f10.4,9x,"NaN",9x,"NaN",9x,"NaN")')lon1,lat1
        endif
       end do
      end do
...
c	 write(6,*)ivin,r,rdis(1),rdis(11)
         ...
         enddo
c         if (r.eq.rcmb) ir=3
      elseif (ivin.eq.2.or.ivin.eq.3.or.ivin.eq.5) then
c for ak135 mdoel
           return
      endif
       end
