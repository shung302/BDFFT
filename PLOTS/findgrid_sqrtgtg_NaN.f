...
c
     ...
     ,          mface=mnx*mny,mvolu=mface*mnz,mdata=100000,mpath=1000)
      ...
      double precision gtg(mvolu),x,y,z,perc,
     ,                 image(mface),ima_max,ima_min
      ...
      write(*,*)lx,ly,lz
      write(*,*)' GTG file to be displayed:'
      read(*,'(a)')ia
      open(1,file=ia,err=73)
      read(1,*,err=73,end=73)(gtg(i),i=1,nvolu)
      close(1)
      write(*,*)'percentage threshold for gtg values?'
      read(*,*) perc
c      gtgmx=-1.d10
c      do i=1,nvolu
...
c      enddo
      ...
      do i=1,ny
       do j=1,nx
        iface=(i-1)*nx+j
        gtg(iface+iz0)=dsqrt(gtg(iface+iz0))
        ima_min=dmin1(ima_min,gtg(iface+iz0))
        ima_max=dmax1(ima_max,gtg(iface+iz0))
       enddo
      enddo
      enddo
      write(6,*)'the max and min of GTG=', ima_max, ima_min
      gtgthres=ima_max*perc
 
 
      open(11,file='NaN.gridindex')
      do ilayer=1,nz
         iz0=(ilayer-1)*nface
      do i=1,ny
       do j=1,nx
        iface=(i-1)*nx+j
        if (gtg(iface+iz0).le.gtgthres) then
           write(11,*)iface+iz0,gtg(iface+iz0)
        endif
       end do
      end do
      end do
