...
c
     ...
     ,          mface=mnx*mny,mvolu=mface*mnz,mdata=60000,mpath=1000)
      ...
      character*200 ia,amdl*5,ia0
      double precision lat0,lon0,lat1,lon1,dp0,dp1,thres
      double precision slow(mvolu),x,y,z,vdiff,
     ,                 image(mface),ima_max,ima_min,slow0(mvolu)
      ...
      open(1,file='NaN.gridindex')
      do i=1,nvolu
         read(1,*,err=72,end=72)idx(i),d
      end do
72    inan=i-1
      close(1)

      write(*,*)' input original velocity model:'
      read(*,'(a)')ia
      ...
      do i=1,inan
         ...
         slow(ii)=9999.0d0
      enddo
      open(11,file='try.xyz')
      do i=1,nvolu
c         if (slow(i).eq.9999.0d0) then
c            write(11,'("NaN")')
c         else
            write(11,*)slow(i)
c         endif
