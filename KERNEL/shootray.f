...
c PKPcd (PKiKP)
     ...
     &     'PcP5','PcP6','pP',
     &     'pPP','Pd','PS','PcS','PKS','PKPab','PKPbc','PKPdf','PKKPdf','PKPcd',
     &     'S','SS','SSS','ScS','ScS2','ScS3','ScS4',
     &     'ScS5','ScS6','sS',
     &     'sSS','Sd','SP','ScP','SKP','SKSac','SKKSac','SKSdf','SKKSdf','SKiKP'/
...
c      REAL*4 t0,p0,d0,t1,p1,del,az
      ...
      CHARACTER*80 fid,fout,fevst,fnm
...
c            alpha=4.0d0*pi*pi/(tpstf(i)*tpstf(i))
            ...
            write(6,*) 'lowest & highest frequencies and tstars for i=1,ntstf'
            read(5,*)wb,we
            read(5,*)(tpstf(i),i=1,ntstf)
            do i=1,ntstf
...
c           endif
     ...
     &     ni,isurf,icmb,iocb,iiob)
           write(6,*)'fvec1 =',fvec1,ia,aii*dpr,yp(3,ni)
c           write(6,*)'iocb=',iocb,iiob,icmb,isurf
     ...
     &        ni,isurf,icmb,iocb,iiob)
              write(6,*)'fvec in ia=',fvec1*fvec2,fvec2,ia,aibd(ia)*dpr,aii*dpr,yp(3,ni)
              if ((fvec1*fvec2).lt.0.0d0) then
                 ...
                 write(6,*)'fvec go to=',fvec1*fvec2,fvec1,fvec2,ia,aibd(ia)*dpr,aii*dpr
...
1500    continue
        write(6,*)'fvec 1500=',ystart(2)*dpr,aii*dpr,dbrk*dpr
...
c       rtbeg=etime(rta)
     ...
     &  dra,dth,dph,doff,distrs,insflag,ntstf,tpstf)
