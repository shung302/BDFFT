...
c ci=c is the velocity at scatterer
      ...
      subroutine kernblk27_vel(ivin,nid,fid,ips,iphase,nlags,lagb,lage,ni,
...
c	       write(6,*)'sk=',sk(istf)*ci,ci,area,dz,wtx,wty,wtz
               ...
               wtw(iii,istf)=ci*sk(istf)*wtx*wty*wtz*area*dz
...
c$            end do
c$            do ii=1,8
c$c              write(92,*)'coef before=',jsta,iposition(ii), coef(iposition(ii),istf),value(ii)
c$              if(jsta.eq.1)then
c$                coef_ref(iposition(ii),istf)=coef_ref(iposition(ii),istf)+value(ii)
c$              else
c$                coef(iposition(ii),istf)=coef(iposition(ii),istf)+value(ii)
c$c                coef(iposition(ii),istf)=value(ii)
c$              endif
c$c              write(92,*)'coef=',jsta,iposition(ii),coef_ref(iposition(ii),istf), coef(iposition(ii),istf)
c$            end do
c$        end do
...
c===================================================================
        ...
        do istf=1,ntstf
           ...
           do ii=1,8
           if (jsta.eq.1) then
              coef_ref(iposition(ii),istf)=coef_ref(iposition(ii),istf)+value(ii)
           else
