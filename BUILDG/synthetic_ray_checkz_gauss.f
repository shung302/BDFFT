...
c            write(6,*) akr(i),rho,akc(i,1),akc(i,2)
      ...
      read(5,*)tstd
      write(6,*)'peak velocity perturbation of a gaussian anomaly'
      read(5,*)dvel
      write(6,*)'travel time demeaned (1=yes)?'
      read(5,*)ifdemean
      call mesh_car(lx,ly,lz,ndata)
      write(6,'(''the grid points in x,y,z directions'',3i6)')nx,ny,nz
      write(6,*)'input 2*sigma+1 in grid points in x, y direction for a gaussian anomaly' 
      write(6,*)'input width of grid points in z direction for checkboard' 
      write(6,*)'enter 2*sigma+1, 2*sigma+1, zwidth and must be odd integers'
      read(5,*)nxc,nyc,nzc
      ...
      do j=-nysig,nysig
         ...
         do i=-nxsig,nxsig
            ii=i+nxsig+1  
            ga(ii,jj)=dexp(-(dble(i)/sigx)**2-(dble(j)/sigy)**2)
c            write(6,*)ii,jj,ga(ii,jj)
         enddo
      enddo 
      do iz=1,nz
         izz=nz-iz+1
         ...
         do iy=1,ny
            ...
            do ix=1,nx
               ...
               ixg=mod1(ixx,nxc)
               ixycsign=(-1)**(iyc+ixc)
               ivolu=(iz-1)*nface+(iy-1)*nx+ix
               slow(ivolu)=-dvel/c*ga(ixg,iyg)*dble(ixycsign*izcsign)
               write(2,'(e12.3)') slow(ivolu)
c               write(2,*) slow(ivolu), ga(ixg,iyg), c, dvel, ixycsign,izcsign
               write(12,'(e12.3)') -slow(ivolu)*c
c             write(6,*)ix,iy,iz,slow(ivolu),ixycsign,izcsign
            enddo
         enddo
      enddo
      close(2)
