...
c     solver_WSE.f:  
c
c       WS is for an ad hoc weighting<weig_sta> and <weig_evt> imposed on 
c       STATION and EVENT CORRECTIONs, respectively
c       
c        output files:
...
c
      implicit none
      character*120 input_matrix
      character*10 mode0(4),mode
      character*5  dumstn,dumnet*2
      integer mlevelx, mlevely, mlevelz, mx, my, mz
      integer mivolu, mbase, mvolu, mdata, mstation, mevent, mvar
      parameter(mlevelx=7,mlevely=7,mlevelz=7,
     ,          mx=2**(mlevelx-1)+1,my=2**(mlevely-1)+1,
     ,          mz=2**(mlevelz-1)+1,mivolu=mx*my*mz,
     ,          mbase=1,mvolu=mivolu*mbase,mdata=200000,mstation=1000,
     ,          mevent=10000,mvar=mvolu+mstation+mevent)
     ...
     ,       mean,vv1(mvar),weig_sta,weig_evt,sigmaz,vmin_sta,vmax_sta,
     ,       vmin_evt,vmax_evt,image2D(mx*my),chi2
c Y.Shen
c      double precision gtgdiag(mvar)
c
      real data_thres
      double precision gtgthres
      integer imultis, ndat, nx, ny, nz, nsta,nevt
      integer nx0, ny0, nz0, nvolust, nvar, ii
      real rhs_min, rhs_max
      integer ndat_act, k, iiz, iiz0, iiy, iiy0, iix, nit 
      integer nstation, nevent
      real plat,plon
      ...
      read(*,'(a)')input_matrix
      write(6,*)'station terms solved simultaneously & weighting'
      read(5,*)weig_sta
      write(*,*) weig_sta
      write(6,*)'event terms solved simultaneously & weighting'
      read(5,*)weig_evt
      write(6,*)weig_evt
      open(1,file=input_matrix,form='unformatted',err=1001)
c      read(1,err=1001,end=1001)ndat,nlevel(1),nlevel(2),nlevel(3),nsta
      read(1,err=1001,end=1001)ndat,nx,ny,nz,nsta,nevt
      write(*,*)ndat,nx,ny,nz,nsta,nevt

      nlevel(1)=nint(log(real(nx-1))/log(2.0))+1
      nlevel(2)=nint(log(real(ny-1))/log(2.0))+1
      nlevel(3)=nint(log(real(nz-1))/log(2.0))+1
      write(6,*)'level=',(nlevel(i),i=1,3)
      nx0=2**(nlevel(1)-1)+1
      ny0=2**(nlevel(2)-1)+1
      nz0=2**(nlevel(3)-1)+1
     ...
     .                    nlevel(3).gt.mlevelz) then
       ...
       write(*,*)mdata,mlevelx,mlevely,mlevelz,nsta,nevt
...
c      nz=2**(nlevel(3)-1)+1
      ...
      nvolu=nivolu
      nvolust=nvolu+nsta
      nvar=nvolu+nsta+nevt
      write(*,*)ndat,(nlevel(i),i=1,3),nx,ny,nz,nvolu,nsta,nevt
      if(imultis.eq.0)then
       open(79,file='results',status='old',form='unformatted',
     ,                        err=1002)
       read(79,err=1002)ii,nsta,lamb,nevt
       mode=mode0(ii)
      ...
      elseif(imultis.ge.3)then
       ...
       write(79,err=1002)imultis,nsta,lamb,nevt
      else
       mode=mode0(imultis)
       open(79,file='results',form='unformatted',err=1002)
       write(79,err=1002)imultis,nsta,lamb,nevt
...
c          call face_lift_Pt3(image,2,nlevel_syn)
         ...
         do j=1,nvar
          if(j.le.nvolu)then
           ...
           ttmax=dmax1(ttmax,dabs(dble(image0(j))))
          endif
         end do
         ncoef=0
         do j=1,nvar
          if(j.le.nvolu)then
           tmp=image(j)
          elseif (j.gt.nvolu.and.j.le.nvolust) then
           tmp=dble(image0(j))*weig_sta
          else
           tmp=dble(image0(j))*weig_evt
          endif
...
c      Dampping.....
       ...
       do ii=nvolu+1,nvolu+nsta
        vv1(ii)=vv1(ii)*weig_sta
        errx(ii)=errx(ii)*weig_sta
       end do
       do ii=nvolust+1,nvar
        vv1(ii)=vv1(ii)*weig_evt
        errx(ii)=errx(ii)*weig_evt
       end do

cc      direct examination of fitting...
c       open(1,file=input_matrix,form='unformatted')
c       read(1)ndat,nlevel(1),nlevel(2),nlevel(3),nsta
c       fit=0.d0
c       fit_tmp=0.d0
c       do i=1,ndat
c        read(1)ncoef,(jdx0(j),coef0(j),j=1,ncoef),rhs
c        if(abs(rhs).le.data_thres)then
c         do j=1,nvar
c          if(j.le.nvolu)then
c           image(j)=0.d0
c          endif
c          image0(j)=0.
c         end do
c         do j=1,ncoef
c          k=jdx0(j)
c          if(k.le.nvolu)then
c           image(k)=dble(coef0(j))
c          endif
c          image0(k)=coef0(j)
c         end do
c         if(imultis.eq.2)then
c          call face_lift_Hybrid_Nbt(image,2,nlevel_syn)
cc          call face_lift_Pt3(image,2,nlevel_syn)
c         elseif(imultis.eq.3)then
c          call quell(image)
c         elseif(imultis.eq.4)then
c          call quell(image)
c          do iiz=1,nz
c           iiz0=(iiz-1)*nx*ny
c           do iiy=1,ny
c            iiy0=(iiy-1)*nx
c            do iix=1,nx
c             image2D(iiy0+iix)=image(iiz0+iiy0+iix)
c            enddo
c           enddo
c           call face_liftLC(image2D,2,nlevel_syn2D)
c           do iiy=1,ny
c            iiy0=(iiy-1)*nx
c            do iix=1,nx
c             image(iiz0+iiy0+iix)=image2D(iiy0+iix)
c            enddo
c           enddo
c          enddo
c         end if
c         do j=1,nvolu
c          image0(j)=sngl(image(j))
c         end do
c         fit_tmp=fit_tmp+dble(rhs)*dble(rhs)
c         tmp=0.d0
c         do j=1,nvar
c          tmp=tmp+dble(image0(j))*vv1(j)
c         end do
c         fit=fit+(tmp-dble(rhs))*(tmp-dble(rhs))
c        endif
c       end do
...
c      in the original space
      ...
      read(1)ndat,nlevel(1),nlevel(2),nlevel(3),nsta,nevt

	write(*,*) ndat, nlevel(1),nlevel(2),nlevel(3),nsta,nevt
...
c       endif
       ...
       do j=nvolu+1,nvolust
	vmin_sta=dmin1(vv1(j),vmin_sta)
        vmax_sta=dmax1(vv1(j),vmax_sta)
       end do
       do j=nvolust+1,nvar
	vmin_evt=dmin1(vv1(j),vmin_evt)
        vmax_evt=dmax1(vv1(j),vmax_evt)
       end do
...
c       write(*,*) 'number of points with gtgdiag > ',gtgthres, nk
      ...
      close(78)
      open(78,file='stations_used',err=400)
      read(78,*,err=400,end=400)nstation
      write(6,*)'nstation,nsta=',nstation,nsta
      open(80,file='events_used',err=400)
      read(80,*,err=400,end=400)nevent
      write(6,*)'nevent=',nevent,nevt
      if (nstation.ne.nsta) 
     &  write(6,*)'station numbers are not consistent',nstation,nsta
      if (nevent.ne.nevt) 
     &  write(6,*)'event numbers are not consistent',nevent, nevt

      open(79,file='station_correction')
      do i=1,nsta
c       read(78,'(5x,a5,3x,f11.6,f12.6)',err=400)dumstn,plon,plat
       read(78,*,err=400) dumnet,dumstn,plat,plon
       write(6,*)dumnet,dumstn,plat,plon
       write(79,'(2f10.4,e14.5,i5,1x,a5)')plon,plat,vv1(nvolu+i),i,dumstn
      end do
      close(78)
      close(79)
      open(79,file='event_correction')
      do i=1,nevt
         read(80,*,err=400)plat,plon
         write(6,*)plat,plon
         write(79,'(2f10.5,e14.5)')plon,plat,vv1(nvolust+i)
      enddo
      close(79)
      close(80)
...
100   format(10h var_redu.,10h model var,10h modelnorm,
     ,       6x,4hmean,7x,3hmin,7x,3hmax,6x,4hSmin,6x,4hSmax,6x,4hChi2)
      write(77,*)fit,cov,norm,mean,vmin,vmax,vmin_sta,vmax_sta,chi2
      write(77,*)' '
...
c      write(*,*)'  under the same mode !!'
      write(*,*)' '
      write(*,100)
      write(*,'(9e10.2)')fit,cov,norm,mean,vmin,vmax,vmin_sta,vmax_sta,chi2
      write(*,*)' '
