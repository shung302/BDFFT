...
#foreach damp ( 10 20 50 100 200 500 1000 2000 3000 4000 5000 7000 10000 20000 50000 )
foreach damp ( 50 100 300 200 500 1000 2000 3000 4000 5000 7000 10000 20000 50000 )                                          

#echo "damp = $damp , output = try.simple_kern_se"$ws-$we"_$damp.$F.$mdl"
echo "Inversioning....... Damping:$damp"
set weight = "$ws-$we"
#if ( -f try.simple_kern_se"$weight"_"$damp".$F.$mdl ) continue
...
$we
!
mv try.xyz try.simple_kern_se"$weight"_"$damp".$F.$mdl
mv try.sum sum.simple_kern_se"$weight"_"$damp".$F.$mdl
mv results results.simple_kern_se"$weight"_"$damp".$F.$mdl
mv station_correction stacor.simple_kern_se"$weight"_"$damp".$F.$mdl
mv event_correction evtcor.simple_kern_se"$weight"_"$damp".$F.$mdl
end #foreach damp
end #foreach ws
